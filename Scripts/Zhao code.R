...
sdnm_mcmc <- function(y, x, w, dist, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    ### Sample beta0 (spp. interactions coefficients)
    ...
    for (s in 1:nspec) {
      ...
      mh1 <- sum(dpois(N[,1,s], lambda0_star[,s], log=T)) + sum(dnorm(beta0_star[s,], beta0_mean[s,], beta0_sd, log=T))
      
      mh2 <- sum(dpois(N[,1,s], lambda0[,s], log=T)) + sum(dnorm(beta0[s,], beta0_mean[s,], beta0_sd, log=T))
