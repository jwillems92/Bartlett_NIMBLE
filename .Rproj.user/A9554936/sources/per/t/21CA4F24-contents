...
mast <- read_csv("Data/beech_mast.csv") # already scaled
snwd <- read_csv("Data/Heimal Days Conway.csv")
tmin <- read_csv("Data/Conway Average TMIN Deviation vs Longterm.csv")
precip <- read_csv("Data/NConway_CumulPrecip_Daily.csv")
herbs <- read_csv("Data/Mean Total Herbaceous Cover.csv")
rsd <- read_csv("Data/Combined RSD.csv")
trap_df <- read_csv("Data/trap_number.csv")

# Scale precip data & put into 2D matrix
precip.mat <- precip %>% 
  mutate(YEAR = YEAR-1993,
         cumul_precip = as.numeric(scale(cumul_precip)),
         TP = paste0("TP",TP)) %>% 
  pivot_wider(names_from = YEAR, values_from = cumul_precip) %>% 
  select(-c(TP)) %>% 
  #column_to_rownames(var="YEAR") %>% 
  as.matrix()

rownames(precip.mat) <- c(1,2,3)

# Group year-level covariates together & z-score
yearly.covs <- cbind(mast, tmin$mean_TMIN_deviation, snwd$TotalHDays) %>% 
  rename("Year" = 1,
         "mast" = 2,
         "tmin" = 3,
         "snwd" = 4) %>% 
  mutate(tmin = as.numeric(scale(tmin)),
         snwd = as.numeric(scale(snwd)))

# Format site-level covs (replace missing values with overall mean value)
all.year.comps <- merge(unique(rsd$YEAR), unique(rsd$COMP)) %>% rename("YEAR" = 1, "COMP" = 2)
all.years.rsd <- left_join(all.year.comps,rsd)
site.covs <- left_join(all.years.rsd,herbs) %>% 
  mutate_all(~ifelse(is.na(.x), mean(.x, na.rm = TRUE), .x)) %>% 
  mutate(rsd = as.numeric(scale(RSD)),
         herbs = as.numeric(scale(MeanTotalHerb))) %>% 
  select(-c(RSD, MeanTotalHerb))

# Put rsd and herbs into 2D matrices
rsd.mat <- site.covs %>% select(YEAR, COMP, rsd) %>% 
  pivot_wider(names_from = COMP, values_from = rsd) %>% 
  column_to_rownames(var="YEAR") %>% 
  as.matrix() 

herbs.mat <- site.covs %>% select(YEAR, COMP, herbs) %>% 
  pivot_wider(names_from = COMP, values_from = herbs) %>% 
  column_to_rownames(var="YEAR") %>% 
  as.matrix()
...
#write_rds(out1, file="BEF_Nimble_AllSpp_5k.rds")

out.sum <- MCMCsummary(out1, round=2)
out.sum
...
befNim1 <- nimbleModel(code = nimbleBEF1, 
                       constants = model.constants, 
                       data = model.data, 
                       inits = inits)

befNim1$initializeInfo()
befNim1$calculate("phi")
...
samps.df <- MCMCsummary(samples, round=2)
